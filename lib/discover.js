...
function addDiscover (content, tags, new_tags, mentions, selectedAddress, position, successCallback) {
  ...
  let data = {
    ...
    current_address_longitude: selectedAddress && selectedAddress !== '不显示位置' && selectedAddress !== '所在位置' ? position.longt : '',
    current_address_latitude: selectedAddress && selectedAddress !== '不显示位置' && selectedAddress !== '所在位置' ? position.lat : '',
    group_id: 0
  }

  postRequest(`article/store`, data, true).then(res => {
    var code = res.code
    if (code === 6101) {
      // 已存在
