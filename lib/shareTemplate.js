...
function getAskCommunityMajorDetail (questionId, questionDescription, answerUsername) {
  ...
  var link = store.state.webRoot + '/#' + '/askCommunity/major/' + questionId + '?uuid=' + getLocalUuid()
...
function getAskCommunityInteractionDetail (answerId, questionDescription, answerUsername) {
  ...
  var link = store.state.webRoot + '/#' + '/ask/offer/' + answerId + '?uuid=' + getLocalUuid()
...
function getAskCommunityInteractionAnswers (questionId, description, answerNum, followNum) {
  ...
  var link = store.state.webRoot + '/#' + '/ask/offer/answers/' + questionId + '?uuid=' + getLocalUuid()
...
function getInvitation (username, rcCode) {
  var link = store.state.webRoot + '/#' + '/invitation/register?rc_code=' + rcCode + '&uuid=' + getLocalUuid()
...
function getDiscoverDetail (pathUrl, title, imgUrl) {
  ...
  var link = store.state.webRoot + '/#' + pathUrl + '&noback=1' + '&uuid=' + getLocalUuid()
...
function getResumeDetail (uuid, realname, companyName, avatarUrl) {
  var link = store.state.webRoot + '/#/share/resume/' + uuid + '?time=' + (new Date().getTime()) + '&uuid=' + getLocalUuid()
...
function getGroupDetail (id, title, username, userCount, logo) {
  var link = store.state.webRoot + '/#' + '/group/detail/' + id + '?uuid=' + getLocalUuid()
...
function getRecommends (count) {
  var link = store.state.webRoot + '/#' + '/recommends' + '?uuid=' + getLocalUuid()
...
function getDianpingCommentDetail (slug, title, imgUrl, username, rateStar) {
  // var link = process.env.API_ROOT + 'wechat/oauth?redirect=' + pathUrl + encodeURIComponent('?noback=1')
  var link = store.state.webRoot + '/#' + '/dianping/comment/' + slug + '?noback=1&uuid=' + getLocalUuid()
...
function getDianpingProductDetail (username, productName, productDesc, imgUrl) {
  // var link = process.env.API_ROOT + 'wechat/oauth?redirect=' + pathUrl + encodeURIComponent('?noback=1')
  var link = store.state.webRoot + '/#' + '/dianping/product/' + encodeURIComponent(productName) + '?noback=1&uuid=' + getLocalUuid()
