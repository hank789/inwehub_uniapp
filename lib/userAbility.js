...
import { getLocalUserInfo, isCompanyStatus, getLocalUuid } from '@/lib/user'
import { postRequest } from '@/lib/request'
import { isLogined } from '@/lib/auth'
import { joinIn } from '@/lib/group'
...
var UserAbility = () => {
  ...
  var newbieTask = (context, id) => {
    ...
    var num = parseInt(localEvent.get('num' + mobile).value)
    if (num !== 1) {
  ...
  var signIGift = (context, callback) => {
    ...
    var mobile = userInfo.phone
    if (mobile) {
      // localEvent.set('sign_day_' + mobile, {value: '2018-4-1'})
      var tody = new Date()
      var isTody = tody.getFullYear() + '-' + (tody.getMonth() + 1) + '-' + tody.getDate()
      var day = localEvent.get('sign_day_' + mobile).value
      // if (day !== isTody) {
          ...
          if (day !== isTody) {
            if (num.index >= 0) {
              postRequest('activity/sign/daily', {}).then(response => {
                ...
                // 签到请求成功
                localEvent.set('sign_day_' + mobile, { value: isTody })
                if (response.data.coupon_type === 0) {
                  // 积分奖励弹窗
                  var signDaily = response.data
                  alertGetCredits(context, signDaily)
                } else {
                  // 红包请求
                  postRequest('activity/getCoupon', { coupon_type: response.data.coupon_type }).then(response => {
                    var code = response.code
                    if (code !== 1000) {
                      ui.toast(response.message)
                      return
  ...
  function showFreeAskGuide(context) {
    var history = parseInt(localEvent.get('showFreeAskGuide_' + getLocalUuid()))
    if (!history) {
      localEvent.set('showFreeAskGuide_' + getLocalUuid(), 1)
  ...
  var jumpToDianpingAdd = (context, productName, rating = 0) => {
    var isValid = userAbilityCheck.checkPhoneCertification(context)
    if (isValid) {
      uni.navigateTo({url: '/pages/dianping/add?id=' + productName + '&rating=' + rating})
    }
  }
