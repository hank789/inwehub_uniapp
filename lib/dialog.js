var windowDialogQueue = []
var windowDialogHtmlQueue = []
var windowDialogCurrentHtml = ''
...
function isHaveAlert (callback, html) {
  ...
  if (alert) {
    // 防止重复
    if (html === windowDialogCurrentHtml) {
      console.log('发现与当前弹窗重发，自动忽略')
      return true
    }
    for (var i = 0; i < windowDialogHtmlQueue.length; i++) {
      if (windowDialogHtmlQueue[i] === html) {
    ...
    windowDialogHtmlQueue.push(html)
    windowDialogQueue.push(callback)
    return true
  }
  windowDialogCurrentHtml = html
  return false
}
...
function continueAlert () {
  if (windowDialogQueue.length) {
    var callback = windowDialogQueue.shift()
