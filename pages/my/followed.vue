<template>
	<view class="content">
		<iwList v-model="list" :api="'followed/users'" @scrollList="scrollList" ref="iwList" :cssTop="cssTop" :requestData="feedListParams">
			...
			<view class="myFocus">
				<view class="focusItem" v-for="(item, index) in list" :key="index">
					...
					<view class="follows" @tap.stop.prevent="collectProfessor(item, index)" v-if="!item.is_following">关注Ta</view>
					<view class="follows bgblue" @tap.stop.prevent="collectProfessor(item, index)" v-else>已关注</view>
					<text class="bot"></text>
				</view>
			</view>
		</iwList>
