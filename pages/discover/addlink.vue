<template>
    <view>
        <cmd-nav-bar :fixed="false" right-color="#03AEF9" right-text="确认发布" back title="发布" @rightText="rightText"></cmd-nav-bar>
        ...
        <view v-if="!loading">
            ...
            <view class="linkWrapper" v-show="urlTitle">
                <view class="leftText">
                    <view class="text font-family-medium text-line-2">{{ urlTitle }}</view>
                    <text class="iconfont icon-times--"></text>
                </view>
                <view class="right" v-show="urlImage">
                    <view class="articleImg">
                        <image :src="urlImage" width="97" height="71"></image>
                    </view>
                </view>
            </view>
            ...
            <view class="tagsWapper">
                ...
                <view class="tagsList">
                    <text class="span" v-for="(item, index) in regions" v-if="item.value>0" :class="{active: item.selected}" @tap.stop.prevent="choiceItem(index, item)">{{item.text}}</text>
                </view>
            </view>
